    ...
    */
	...
	Calendar.prototype._initDayEvents = function(day) {
		...
		instance.clickFn = function(ev) {
			...
			self.isAnimating = true;
			self.isOpen = true;
			self.currentDayIdx = instance.pos;
			self.contentNum = instance.number;
			...
			for(var i = 0, totalDays = self.days.length; i < totalDays; ++i) {
				...
				anime({
					...
					rotateY: isCurrent ? -180 :
                        function () { return anime.random(-180, 180); }
				});
			}
		};
	...
	Calendar.prototype._showPreviewTitle = function(text, number) {
		...
		this.dayPreview.setAttribute('data-number', parseInt(number));
	...
	Calendar.prototype._showContent = function(day) {
	    ...
	    try {
	        var content = contents[this.currentDayIdx],
                ...
                activity = content.querySelectorAll('.content__activity'),
                description = content.querySelectorAll('.content__description'),
                meta = content.querySelectorAll('.content__meta'),
	            editContent = content.querySelectorAll('.content__edit'),
	            delContent = content.querySelectorAll('.content__delete');
	    } catch (e) {
       ...
       anime({
               targets: [activity, description, meta, editContent, delContent],
			    duration: 800,
		...
		contentNumber.innerHTML = (parseInt)(this.contentNum);
		anime({
